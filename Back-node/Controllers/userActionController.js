    ...
    if (req.method === 'POST') {
      ...
      if (postData) {
        ...
        if (postData.posX !== undefined) {
          // Crie uma instância do modelo de ação do usuário
          const userActionModel = new UserActionModel();
          ...
          const success = userActionModel.insertUserAction(dataToInsert);
  ...
  updateAllItems: (req, res) => {
    if (req.method === 'PUT') {
      ...
      if (putData && putData.userAction_id !== undefined) {
        const userActionModel = new UserActionModel();
        const success = userActionModel.updateUserAction(putData);
