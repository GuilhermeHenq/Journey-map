    ...
    if (req.method === 'POST') {
      ...
      if (postData) {
        ...
        if (postData.posX !== undefined) {
          // Crie uma instância do modelo de fase da jornada 
          const journeyPhaseModel = new JourneyPhaseModel();
          ...
          const success = journeyPhaseModel.insertJourneyPhase(dataToInsert);
  ...
  updateAllItems: (req, res) => {
    if (req.method === 'PUT') {
      ...
      if (putData && putData.journeyPhase_id !== undefined) {
        const journeyPhaseModel = new JourneyPhaseModel();
        const success = journeyPhaseModel.updateJourneyPhase(putData);
